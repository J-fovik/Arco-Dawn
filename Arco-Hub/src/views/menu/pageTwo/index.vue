<template>
	<div>
		...
		>
			...
			<template #operate>
				<a-space>
					<a-button type="outline" @click="$router.push(`/menu/pageTwo/operate`)">
						<template #icon>
							<icon-plus />
						</template>
						新增
					...
					<template #operate="{ record }">
						<a-button-group v-if="record.sortTableNo !== '总计'">
							<a-button
								...
								@click="
									$router.push(
										`/menu/pageTwo/operate?id=${record.id}&isReadOnly=true`
									)
								"
								>详情</a-button
							>
							...
							<a-button
								type="text"
								@click="$router.push(`/menu/pageTwo/operate?id=${record.id}`)"
								>编辑</a-button
							>
						</a-button-group>
					</template>
